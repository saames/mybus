    ...
    try:
        ...
        if pontos_da_rota:
            ...
            for i in range(len(nome_rotas)):
                ...
                map_widget.set_marker(paradas[i][0], paradas[i][1])
...
def geocodificar():
    ...
    data = r.json()
    if data:
        paradas.append((float(data[0]["lat"]), float(data[0]["lon"])))
        map_widget.set_marker(float(data[0]["lat"]), float(data[0]["lon"]), endereco.get())
        endereco.set("")



endereco = tkinter.StringVar()
input_widget = tkinter.Entry(app, textvariable=endereco)
input_widget.pack()
btn_endereco = tkinter.Button(app, text="Buscar endereço", command=geocodificar)
btn_endereco.pack()
